                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('role') is-invalid @enderror" id="role" name="role">
                                        <option value="user" {{ (old('role', $employee->role) == 'user') ? 'selected' : '' }}>Employee</option>
                                        <option value="admin" {{ (old('role', $employee->role) == 'admin') ? 'selected' : '' }}>Administrator</option>
                                        <option value="superadmin" {{ (old('role', $employee->role) == 'superadmin') ? 'selected' : '' }}>Super Administrator</option>
                                    </select>
                                    @error('role')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                    ...
                    <div class="col-md-12">
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="date" class="form-control @error('hire_date') is-invalid @enderror" 
                                            id="hire_date" name="hire_date" value="{{ old('hire_date', $employee->hire_date ? $employee->hire_date->format('Y-m-d') : '') }}">
                                        @error('hire_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                @if($employee->employment_status == 'terminated' || old('employment_status') == 'terminated')
                                <div class="form-row" id="termination_date_row">
                                    <div class="form-group col-md-6">
                                        <label for="termination_date">Termination Date</label>
                                        <input type="date" class="form-control @error('termination_date') is-invalid @enderror" 
                                            id="termination_date" name="termination_date" value="{{ old('termination_date', $employee->termination_date ? $employee->termination_date->format('Y-m-d') : '') }}">
    ...
    $(document).ready(function() {
        ...
        $('#employment_status').change(function() {
            if ($(this).val() === 'terminated') {
                if ($('#termination_date_row').length === 0) {
                    let terminationRow = `
                        <div class="form-row" id="termination_date_row">
                            <div class="form-group col-md-6">
                                <label for="termination_date">Termination Date</label>
                                <input type="date" class="form-control" id="termination_date" name="termination_date" value="{{ old('termination_date', $employee->termination_date ? $employee->termination_date->format('Y-m-d') : '') }}">
                            </div>
                        </div>
                    `;
                    $(this).closest('.form-row').after(terminationRow);
                }
            } else {
                $('#termination_date_row').remove();
            }
        });
