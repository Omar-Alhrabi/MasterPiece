        ...
        <div class="card-body">
            <form action="{{ route('payroll.update', $salary) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" 
                                    id="amount" name="amount" value="{{ old('amount', $salary->amount) }}" required>
                            </div>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                                ...
                                <option value="basic" {{ old('type', $salary->type) == 'basic' ? 'selected' : '' }}>Basic Salary</option>
                                <option value="bonus" {{ old('type', $salary->type) == 'bonus' ? 'selected' : '' }}>Bonus</option>
                                <option value="allowance" {{ old('type', $salary->type) == 'allowance' ? 'selected' : '' }}>Allowance</option>
                                <option value="deduction" {{ old('type', $salary->type) == 'deduction' ? 'selected' : '' }}>Deduction</option>
                                <option value="overtime" {{ old('type', $salary->type) == 'overtime' ? 'selected' : '' }}>Overtime</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                id="description" name="description" rows="3" required>{{ old('description', $salary->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control @error('year') is-invalid @enderror" id="year" name="year" required>
                                    ...
                                    @foreach(range(date('Y') - 2, date('Y') + 1) as $year)
                                        <option value="{{ $year }}" {{ old('year', $salary->year) == $year ? 'selected' : '' }}>
                                            {{ $year }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('payment_date') is-invalid @enderror" 
                                id="payment_date" name="payment_date" value="{{ old('payment_date', $salary->payment_date->format('Y-m-d')) }}" required>
                            @error('payment_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('is_paid') is-invalid @enderror" id="is_paid" name="is_paid" required>
                                <option value="1" {{ old('is_paid', $salary->is_paid) == '1' ? 'selected' : '' }}>Paid</option>
                                <option value="0" {{ old('is_paid', $salary->is_paid) == '0' ? 'selected' : '' }}>Unpaid</option>
                            </select>
                            @error('is_paid')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="text-center mt-4">
                    <button type="submit" class="btn btn-primary px-5">Update Salary Record</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        // Trigger the change event if deduction is already selected
